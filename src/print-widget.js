...
function drawPrintWidget(){
  ...
  document.getElementById('pw-go').onclick=()=>{
    ...
    const logo=c.logo?'<img src="'+c.logo+'" style="max-height:70px;max-width:160px;object-fit:contain">':'';
    const company='<div>'+logo+'<h2>'+String(c.name||'Compagnie')+'</h2><p>'+String(c.address||'')+'<br>'+String(c.phone||'')+'<br>'+String(c.email||'')+'<br>Taxe: '+String(c.tax||'')+'<br>WCB: '+String(c.wcb||'')+'<br>Assurance: '+String(c.insurance||'')+'</p></div>';
    ...
    const html='<!doctype html><html><head><title>'+num+'</title><style>body{font-family:Arial;color:#111;padding:32px}header{display:flex;justify-content:space-between;gap:24px;border-bottom:3px solid #111;padding-bottom:16px}h1{font-size:36px;margin:0}h2{margin:0 0 8px}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{border:1px solid #ddd;padding:10px;text-align:left}.total{font-size:24px;font-weight:900;text-align:right;margin-top:24px}.sig{margin-top:60px;width:320px;border-top:2px solid #111;padding-top:10px}.client{text-align:right}</style></head><body><header>'+company+'<div class="client"><h1>Document '+num+'</h1><b>Client:</b> '+String(doc.Client||'')+'<br><b>Chantier:</b> '+String(doc.Chantier||'')+'<br><b>Statut:</b> '+String(doc.Statut||'')+'</div></header><table><tr><th>Description</th><th>Qte</th><th>Unite</th><th>Prix</th><th>Total</th></tr>'+rows+'</table><div class="total">Total net: '+pwNet(doc).toLocaleString('fr-CA',{style:'currency',currency:'CAD'})+'</div><p>Notes: '+String(doc.Notes||'')+'</p><div class="sig">Signature: '+sign+'<br>Date: '+sdate+'</div></body></html>';
